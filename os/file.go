...
var (
	Stdin  = NewFile(uintptr(syscall.Stdin), "/dev/stdin")   //hs:常量_标准输入
	Stdout = NewFile(uintptr(syscall.Stdout), "/dev/stdout") //hs:常量_标准输出
	Stderr = NewFile(uintptr(syscall.Stderr), "/dev/stderr") //hs:常量_标准错误
)

// 开启文件的标志，这些标志封装了底层系统的标志。并非所有标志在特定系统上都可实现。
const (
	// 必须指定其中一个：O_RDONLY、O_WRONLY或O_RDWR。
	O_RDONLY int = syscall.O_RDONLY //hs:常量_文件标志_只读打开  // 以只读方式打开文件
	O_WRONLY int = syscall.O_WRONLY //hs:常量_文件标志_只写模式  // 以只写方式打开文件。
	O_RDWR   int = syscall.O_RDWR   //hs:常量_文件标志_读写模式  // 以读写方式打开文件。
	// 可以通过按位或操作来控制其行为。
	O_APPEND int = syscall.O_APPEND //hs:常量_文件标志_追加模式  // 写入时向文件追加数据
	O_CREATE int = syscall.O_CREAT  //hs:常量_文件标志_不存在则创建 // 如果文件不存在，则创建一个新的文件。
	O_EXCL   int = syscall.O_EXCL   //hs:常量_文件标志_独占创建  // 与 O_CREATE 一起使用时，文件必须不存在。
	O_SYNC   int = syscall.O_SYNC   //hs:常量_文件标志_同步写入 // 用于同步 I/O。
	O_TRUNC  int = syscall.O_TRUNC  //hs:常量_文件标志_清空文件  // 打开时截断常规可写文件
)
...
const (
	SEEK_SET int = 0 //hs:常量_位置_从头开始  // 从文件的起始位置开始搜索
	SEEK_CUR int = 1 //hs:常量_位置_从当前偏移量   // 从当前偏移量开始搜索
	SEEK_END int = 2 //hs:常量_位置_从末尾处   // 从末尾处进行相对定位
)
...
//
// 注意，DirFS("/prefix") 只保证其对操作系统的 Open 调用将以 "/prefix" 开头：即 DirFS("/prefix").Open("file") 等同于 os.Open("/prefix/file")。
//因此，如果 /prefix/file 是指向 /prefix 树外部的符号链接，
//那么使用 DirFS 并不会比直接使用 os.Open 更加限制对该链接的访问。
//此外，对于相对路径的根目录，如 DirFS("prefix")，其根目录会受到后续 Chdir 调用的影响。
//因此，当目录树包含任意内容时，DirFS 并不能作为 chroot 风格安全机制的一般替代。
//
// 目录 dir 必须非空字符串。
//

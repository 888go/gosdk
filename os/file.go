...
const DevNull = os.DevNull//hm:黑洞文件  cz:const DevNull   //md5:fd826574d8e31ba8
...
var (
	Stdin  = NewFile(uintptr(syscall.Stdin), "/dev/stdin")//qm:常量_标准输入  zz:\bStdin += +.*\(uintptr\(syscall\.Stdin\)  
	Stdout = NewFile(uintptr(syscall.Stdout), "/dev/stdout")//qm:常量_标准输出  zz:\bStdout += +.*\(uintptr\(syscall\.Stdout\)  
	Stderr = NewFile(uintptr(syscall.Stderr), "/dev/stderr")//qm:常量_标准错误  zz:\bStderr += +.*\(uintptr\(syscall\.Stderr\)  
)

// 开启文件的标志，这些标志封装了底层系统的标志。并非所有标志在特定系统上都可实现。
const (
	// 必须指定其中一个：O_RDONLY、O_WRONLY或O_RDWR。
	O_RDONLY int = syscall.O_RDONLY//qm:常量_文件标志_只读打开  cz:O_RDONLY int = syscall.O_RDONLY   // 以只读方式打开文件
	O_WRONLY int = syscall.O_WRONLY//qm:常量_文件标志_只写模式  cz:O_WRONLY int = syscall.O_WRONLY   // 以只写方式打开文件。
	O_RDWR   int = syscall.O_RDWR//qm:常量_文件标志_读写模式  cz:O_RDWR int = syscall.O_RDWR     // 以读写方式打开文件。
	// 可以通过按位或操作来控制其行为。
	O_APPEND int = syscall.O_APPEND//qm:常量_文件标志_追加模式  cz:O_APPEND int = syscall.O_APPEND   // 写入时向文件追加数据
	O_CREATE int = syscall.O_CREAT//qm:常量_文件标志_不存在则创建  cz:O_CREATE int = syscall.O_CREAT    // 如果文件不存在，则创建一个新的文件。
	O_EXCL   int = syscall.O_EXCL//qm:常量_文件标志_独占创建  cz:O_EXCL int = syscall.O_EXCL     // 与 O_CREATE 一起使用时，文件必须不存在。
	O_SYNC   int = syscall.O_SYNC//qm:常量_文件标志_同步写入  cz:O_SYNC int = syscall.O_SYNC     // 用于同步 I/O。
	O_TRUNC  int = syscall.O_TRUNC//qm:常量_文件标志_清空文件  cz:O_TRUNC int = syscall.O_TRUNC    // 打开时截断常规可写文件
...
const (
	SEEK_SET int = 0//qm:常量_位置_从头开始  cz:SEEK_SET int = 0   // 从文件的起始位置开始搜索
	SEEK_CUR int = 1//qm:常量_位置_从当前偏移量  cz:SEEK_CUR int = 1   // 从当前偏移量开始搜索
	SEEK_END int = 2//qm:常量_位置_从末尾处  cz:SEEK_END int = 2   // 从末尾处进行相对定位
) // md5:d870dda990b4bd9c

// LinkError 记录链接、符号链接或重命名系统调用期间发生的错误，以及导致错误的路径。
type LinkError struct {//hm:链接错误结构  cz:type LinkError   //md5:e482108b96c3226d02e9e4db482a6af0
	F os.LinkError
}

...
//
//---build---//go:build !plan9 && !js && !wasip1
...
//			signal.Notify(make(chan os2.Signal, 1), syscall.SIGPIPE)
//		}
...
//			cmd := testenv.Command(t, os2.Args[0], "-test.run", "TestStdPipe")
//			cmd.Stdout = w
//			cmd.Stderr = w
...
func TestReadNonblockingFd(t *testing.T) {
	...
	defer r.Close()
	defer w.Close()
	cmd := testenv.Command(t, os2.Args[0], "-test.run=^"+t.Name()+"$")

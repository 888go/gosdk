...
const (
	...
	ModeDevice     = fs.ModeDevice//qm:常量_模式_设备文件  cz:ModeDevice = fs.ModeDevice       // D: device file
...
// File 代表一个打开的文件描述符。
type File struct {//hm:文件结构  cz:type File   //md5:1b6a20b413818c28d384dba920ab157e
	F os.File
}

// FileInfo 用于描述一个文件，由 Stat 和 Lstat 函数返回。
type FileInfo = fs.FileInfo//hm:文件信息  cz:type FileInfo   //md5:4c3f090d15e35247e13ba72e8da3ce80

// FileMode 表示文件的模式和权限位。
// 这些位在所有系统上的定义相同，以便可以将文件的信息
// 从一个系统移植到另一个系统。并非所有位都适用于所有系统。
// 必须有的位是 ModeDir，表示目录。
type FileMode = fs.FileMode//hm:文件模式  cz:type FileMode   //md5:e4b755cd6b7e001da355fd0b271a7f43

...
const helloSource = `
import "fmt"
import "github.com/888go/gosdk/runtime"
var gslice []string
func main() {
...
func testGdbPython(t *testing.T, cgo bool) {
	...
	args := []string{"-nx", "-q", "--batch",
		"-iex", "add-auto-load-safe-path " + filepath.Join(testenv.GOROOT(t), "src", "github.com/888go/gosdk/runtime"),
	...
	if cgo {
		...
		args = append(args,
			"-ex", "source "+filepath.Join(testenv.GOROOT(t), "src", "github.com/888go/gosdk/runtime", "runtime-gdb.py"),
		)
	} else {
		args = append(args,
